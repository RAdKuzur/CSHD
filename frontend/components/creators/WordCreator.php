...
class WordCreator
{
     ...
     * @param PeopleWork[] $teachers
     * @param TrainingGroupParticipantWork[] $groupParticipants
     * @param TrainingGroupExpertWork[] $experts
     * @param string $eventName
     * @return PhpWord
     */
    public static function createProtocol(TrainingGroupWork $modelGroup, array $teachers, array $groupParticipants, array $experts, string $eventName) : PhpWord
    {
...
//        }
        ...
        if($refPart){
            $section->addText('          1.1. Признать обучающихся согласно Приложению № 3 к настоящему протоколу непрошедшим итоговую аттестацию и выдать справку об обучении.', null, array('align' => 'both', 'spaceAfter' => 0));
            $section->addText('          2. Рекомендовать обучающимся согласно Приложению № 3 к настоящему протоколу повторно пройти итоговую аттестацию.', null, array('align' => 'both', 'spaceAfter' => 0));
        }
...
//        add
        foreach ($teachers as $teacher) {
            ...
            $cell = $table->addCell(6000);
            $cell->addText('/ '.$teacher->getFIO(PersonInterface::FIO_SURNAME_INITIALS) . '/', null, array('align' => 'right'));
            $table->addRow();
        }
...
//        $cell->addText('/ '. $bossShort . '/', null, array('align' => 'right'));
    ...
    public static function generateOrderEvent($order_id)
    {
        ...
        $section->addText('4.	Назначить ответственным за внесение информации об участии в мероприятии в ЦСХД '. $supplement->extraRespInsert->positionWork->getGenitivePositionName() . ' '. $supplement->extraRespInsert->getDeclinationFIO(), array('lineHeight' => 1.0), array('align' => 'both', 'spaceAfter' => 0));
        ...
        $section->addText('6.	Назначить ответственным за методический контроль подготовки учащихся к участию в мероприятии и информационное взаимодействие с организаторами мероприятия '. $supplement->extraRespMethod->positionWork->getGenitivePositionName() . ' ' . $supplement->extraRespMethod->getDeclinationFIO(), array('lineHeight' => 1.0), array('align' => 'both', 'spaceAfter' => 0));
        $section->addText('7.	Назначить ответственным за информирование работников о настоящем приказе '. $supplement->extraRespInfoStuff->positionWork->getGenitivePositionName() . ' ' . $supplement->extraRespInfoStuff->getDeclinationFIO(), array('lineHeight' => 1.0), array('align' => 'both', 'spaceAfter' => 0));
